// lib/explanationmodule/productCardBuilder.ts
import { RankedProduct, ProductCard } from '../../types/product';
...
export class ProductCardBuilder {
  build(
    ...
    const platformInfo = config.platformConfig[platformCode] || {
      name: platformCode.toUpperCase(),
      icon: '🛍️',
    };
    ...
    if (breakdown.credibilityFactor > 0.85) {
    ...
    else if (rankedProduct.matchingFeatures.length)
      short = `✨ Great for ${rankedProduct.matchingFeatures[0]}`;

    // Format overall score to 1 decimal
    const overallScore = parseFloat(rankedProduct.score.toFixed(1));
    ...
    return {
      ...
      rating: {
        score: ratingScore,
        count: rating.count,
        display: `${ratingScore.toFixed(1)}/5`,
        stars,
